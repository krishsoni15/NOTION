...
export function InventoryCard({
    ...
    const handleImageAreaClick = () => {
        if (hasImages) onImageClick(item, currentImageIndex);
    };
    ...
    return (
        <Card className="group relative overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 border border-border bg-card shadow-sm rounded-xl flex flex-col h-full">
            ...
            {/* Image section — only shown if item has images */}
            {hasImages && (
                <div className="relative h-48 w-full overflow-hidden bg-muted flex items-center justify-center group/image">
                    <div className="h-full w-full relative">
                        ...
                        {images.length > 1 && (
                            <>
                                <button onClick={handlePrevImage} className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 hover:bg-black/60 transition-all text-white flex items-center justify-center backdrop-blur-sm z-20 hover:scale-110 border border-white/10" aria-label="Previous Image">
                                    <ChevronLeft className="w-5 h-5" />
                                </button>
                                <button onClick={handleNextImage} className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 hover:bg-black/60 transition-all text-white flex items-center justify-center backdrop-blur-sm z-20 hover:scale-110 border border-white/10" aria-label="Next Image">
                                    <ChevronRight className="w-5 h-5" />
                                </button>
                            </>
                        )}

                        <button className="absolute inset-0 z-0 cursor-zoom-in" onClick={handleImageAreaClick} aria-label="View Fullscreen" />
                                    ...
                                    <PopoverContent className="w-72 p-0 rounded-lg shadow-lg border-border" align="start">
                                        ...
                                        <div className="max-h-[200px] overflow-y-auto p-2 space-y-1">
                                            {(item as any).vendors.map((vendor: any, idx: number) => (
                                                <button key={idx} onClick={() => setSelectedVendorDetails(vendor)} className="w-full text-left p-2 hover:bg-muted/50 rounded-md transition-all text-sm group/vendor border border-transparent hover:border-border/50">
                                    ...
                                    <PopoverContent className="w-72 p-4 rounded-lg shadow-lg border-border" align="start">
                                        <div className="space-y-3">
                                            ...
                                            <div className="pt-2 border-t border-border/50">
                                                <Button variant="ghost" size="sm" className="w-full h-7 text-xs font-medium text-primary hover:text-primary/80 hover:bg-primary/5" onClick={() => setSelectedVendorDetails((item as any).vendor)}>
                                                    View Full Details
                                                </Button>
                                            </div>
                                        </div>
