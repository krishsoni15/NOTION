            ...
            const opt = {
                ...
                html2canvas: {
                    ...
                    onclone: (clonedDoc: Document) => {
                        const regex = /(?:oklch|oklab|lab|lch|color-mix)\((?:[^)(]+|\((?:[^)(]+|\([^)(]*\))*\))*\)/gi;
                        const styleElements = clonedDoc.querySelectorAll('style');
                        styleElements.forEach(el => {
                            if (el.textContent) {
                                el.textContent = el.textContent.replace(regex, '#000000');
                            }
                        });
                        ...
                        for (let i = 0; i < elements.length; i++) {
                            ...
                            if (el.style) {
                                ...
                                if (c.match(regex)) el.style.color = '#000000';
                                if (bg.match(regex)) el.style.backgroundColor = '#ffffff';
                                if (bc.match(regex)) el.style.borderColor = '#000000';
                            }
                        }
                    }
                },
...
Notion Electronica Pvt. Ltd.`;
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DraggableDialogContent
                className="w-[98vw] sm:w-[1400px] max-w-[100vw] h-[95vh] flex flex-col p-0 overflow-hidden border-0 shadow-2xl bg-slate-900 rounded-xl"
                showCloseButton={false}
            >
                {/* Header - Compact, No Overflow */}
                <DialogHeader draggable className="px-2 py-2 border-b border-slate-700/50 bg-slate-800 shrink-0 overflow-hidden">
