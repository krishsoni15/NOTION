...
export function ViewDCDialog({ open, onOpenChange, deliveryId }: ViewDCDialogProps) {
    ...
    if (!deliveryData && open) {
        return (
            <Dialog open={open} onOpenChange={onOpenChange}>
                <DraggableDialogContent className="w-[98vw] sm:w-[1200px] max-w-[100vw] h-[85vh] max-h-[90vh]" showCloseButton={false}>
                    <DialogHeader>
    ...
    return (
        <>
            <Dialog open={open} onOpenChange={onOpenChange}>
                <DraggableDialogContent className="w-[98vw] sm:w-[1200px] max-w-[100vw] h-[85vh] max-h-[90vh] p-0 overflow-hidden" showCloseButton={false}>
                    {/* Header with gradient */}
                    <div data-dialog-drag-handle="true" className="relative cursor-move select-none active:cursor-grabbing border-b bg-background px-6 py-5 text-foreground">
                        <button
                        ...
                        <div className="flex items-start justify-between">
                            <div className="flex items-center gap-3">
                                <div className="w-12 h-12 bg-primary/10 text-primary rounded-xl flex items-center justify-center text-2xl backdrop-blur-sm">
                                    {deliveryTypeIcon}
                                </div>
                                <div>
                                    <h2 className="text-xl font-bold tracking-tight">Delivery Challan</h2>
                                    <p className="text-muted-foreground font-mono text-sm">{deliveryData.deliveryId}</p>
                                </div>
                            </div>
                            ...
                            <Badge
                                className={cn(
                                    ...
                                    deliveryData.status === "delivered" && "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800",
                                    deliveryData.status === "pending" && "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 border-blue-200 dark:border-blue-800",
                                    deliveryData.status === "cancelled" && "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 border-red-200 dark:border-red-800"
                                )}
                            >
                                {statusConfig.label}
                            </Badge>
                        ...
                        {(deliveryData.vendor || deliveryData.po) && (
                            <div className="mt-4 flex flex-wrap gap-4 text-sm">
                                {deliveryData.vendor && (
                                    <div className="flex items-center gap-2 bg-muted px-3 py-1.5 rounded-lg backdrop-blur-sm">
                                        <Building className="h-4 w-4 text-muted-foreground" />
                                        <span className="font-medium text-foreground">{deliveryData.vendor.companyName}</span>
                                    </div>
                                )}
                                {deliveryData.po && (
                                    <div className="flex items-center gap-2">
                                        <div className="flex items-center gap-2 bg-muted px-3 py-1.5 rounded-lg backdrop-blur-sm">
                                            ...
                                            <span className="font-mono text-foreground">{deliveryData.po.poNumber}</span>
                                        </div>
                                        <button
                                            onClick={() => setShowPdfPreview(true)}
                                            className="flex items-center gap-1.5 border bg-background hover:bg-muted px-3 py-1.5 rounded-lg backdrop-blur-sm transition-colors text-sm font-semibold text-muted-foreground hover:text-foreground"
                                        >
