...
import { Zap, Files, Truck, ShoppingBag, PlusCircle, ChevronDown, Table as TableIcon, Search, Filter, LayoutGrid, Check, Package, Clock } from "lucide-react";
...
import { DirectDCDialog } from "@/components/purchase/direct-dc-dialog";
...
export function ManagerRequestsContent() {
    ...
    useEffect(() => {
        ...
        if (view === "cc") {
            setStatusFilter(["ready_for_cc", "cc_pending", "cc_approved"]);
            setEntryTypeFilter("all");
            setWorkFilter("all");
        } else if (view === "po") {
            setStatusFilter(["ready_for_po", "cc_approved", "cc_rejected", "ready_for_delivery", "pending_po", "direct_po", "ordered"]);
            setEntryTypeFilter("all");
            setWorkFilter("all");
        } else if (view === "dc") {
            setStatusFilter(["delivery_processing", "out_for_delivery", "delivered", "ready_for_delivery"]);
            setEntryTypeFilter("all");
            setWorkFilter("all");
        }
    }, [searchParams]);
    ...
    useEffect(() => {
        setCurrentPage(1);
    }, [searchQuery, statusFilter.length, viewMode, entryTypeFilter, workFilter, categoryFilter, inventoryFilter]);
    ...
    const filteredRequests = useMemo(() => {
        ...
        // If filtering for Direct entries only, return empty for real requests
        if (entryTypeFilter === "direct") return [];
    ...
    const groupedAndSortedRequests = useMemo(() => {
        ...
        // ── Merge Direct entries if applicable ──
        if (entryTypeFilter === "all" || entryTypeFilter === "direct") {
            // Merge Direct CCs
            if (directCCs) {
                directCCs.filter(cc => cc.isDirect).forEach(cc => {
                    // Apply status filter
                    if (statusFilter.length > 0 && !statusFilter.includes(cc.status)) return;
                    
                    // Apply search
                    if (searchQuery.trim()) {
                        const q = searchQuery.toLowerCase();
                        const itemName = (cc.directItem?.itemName || "").toLowerCase();
                        if (!itemName.includes(q)) return;
                    }
                    ...
                    sortedGroups.push({
                        ...
                        items: [dummyItem],
                        firstItem: dummyItem,
                        latestUpdate: cc.updatedAt || cc.createdAt,
                        itemCount: 1,
                        isDirectCC: true,
                        directCC: cc
                    } as any);
            ...
            // Merge Direct DCs
            if (directDCDeliveries) {
                directDCDeliveries.forEach(dc => {
                    // Apply status filter
                    const mappedStatus = dc.status === 'pending' ? 'delivery_processing' : dc.status;
                    if (statusFilter.length > 0 && !statusFilter.includes(mappedStatus)) return;

                    // Apply search
                    if (searchQuery.trim()) {
                        const q = searchQuery.toLowerCase();
                        if (!dc.deliveryId.toLowerCase().includes(q) && !dc.vendor?.companyName.toLowerCase().includes(q)) return;
                    }
            ...
            // Re-sort after merging direct items
            sortedGroups.sort((a, b) => b.latestUpdate - a.latestUpdate);
        }

        return sortedGroups;
    }, [filteredRequests, sortBy, entryTypeFilter, directCCs, directDCDeliveries, statusFilter, searchQuery]);
                        ...
                        <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-border/50">
                            ...
                            >
                                <TabsList className="bg-muted/50 border h-full">
                                    ...
                                    <TabsTrigger value="request" className="px-3 py-1 text-xs">Req.</TabsTrigger>
                                    <TabsTrigger value="direct" className="px-3 py-1 text-xs">Dir.</TabsTrigger>
                                </TabsList>
                            </Tabs>
                            ...
                            {/* Split Action Button for Direct Entries */}
                            <div className="relative ml-auto flex items-stretch">
                                <Button
                                    onClick={() => setShowDirectPODialog(true)}
                                    className="bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white shadow-md transition-all h-9 rounded-r-none border-r border-white/20"
                                >
                                ...
                                <Button
                                    ...
                                    className="bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white shadow-md transition-all h-9 rounded-l-none px-2 min-w-0"
                                >
                                ...
                                {splitDropdownOpen && (
                                    <div className="absolute right-0 top-full mt-1 z-50 min-w-[160px] bg-popover border border-border rounded-md shadow-lg py-1 animate-in fade-in-0 zoom-in-95 slide-in-from-top-2 duration-150">
                                        <button
                                            onClick={() => { setSplitDropdownOpen(false); setShowDirectCCDialog(true); }}
                                        ...
                                        <button
                                            onClick={() => { setSplitDropdownOpen(false); setShowDirectDCDialog(true); }}
                                            className="w-full text-left px-3 py-2 text-sm hover:bg-accent hover:text-accent-foreground transition-colors flex items-center gap-2"
                                        >
            ...
            <CostComparisonDialog
                ...
                onOpenChange={(open) => {
                    if (!open) {
                        ...
                        setEditDirectCCId(null);
                    }
                }}
                requestId={!editDirectCCId ? ccRequestId! : undefined}
                requestIds={ccRequestIds}
                isDirect={!!editDirectCCId}
