 ...
 *
 * Shows detailed information about a site location
 */
...
export function SiteInfoDialog({
  ...
  // Get site creator info
  const siteCreator = useQuery(
    api.users.getUserById,
    site?.createdBy ? { userId: site.createdBy } : "skip"
  );
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            {site.code && (
              <div className="flex items-center justify-between">
                <Label className="text-sm font-semibold">Site Code</Label>
                <p className="font-mono text-sm font-medium">{site.code}</p>
              </div>
            )}
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1 flex-1 min-w-0">
                    <p className="text-sm text-right break-words flex-1">{site.address}</p>
                    <button
                      onClick={() => handleOpenInMap(site.address)}
            ...
            <>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <p className="text-sm text-right break-words">{site.description}</p>
                </div>
              </div>
            </>
          )}
          ...
          <div className="flex items-center justify-between">
            ...
            <p className="text-sm">{siteCreator?.fullName || "Unknown"}</p>
          </div>
          ...
          <div className="space-y-2 pt-2 border-t">
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              ...
              <span>{format(new Date(site.createdAt), "dd MMM yyyy")}</span>
            </div>
            {site.updatedAt !== site.createdAt && (
              <div className="flex items-center justify-between text-xs text-muted-foreground">
                <span>Last Updated</span>
                <span>{format(new Date(site.updatedAt), "dd MMM yyyy")}</span>
              </div>
            )}
          </div>
        </div>
