/**
 * Resizable Sticky Notes Sheet Component
 * 
 * Sticky notes sheet that can be resized by dragging the left border.
 */
...
import { NonModalSheetContent } from "@/components/chat/non-modal-sheet-content";
import { VisuallyHidden } from "@/components/ui/visually-hidden";
import { cn } from "@/lib/utils";
import { useChatWidth } from "@/components/chat/chat-width-provider";

interface ResizableStickyNotesSheetProps {
  open: boolean;
...
export function ResizableStickyNotesSheet({ open, onOpenChange, children }: ResizableStickyNotesSheetProps) {
  const [width, setWidth] = useState(DEFAULT_WIDTH);
  const [isResizing, setIsResizing] = useState(false);
  const [isMobile, setIsMobile] = useState(false);
  const sheetRef = useRef<HTMLDivElement>(null);
  const { setStickyNotesWidth, setIsStickyNotesOpen } = useChatWidth();
  ...
  useEffect(() => {
    const savedWidth = localStorage.getItem("sticky-notes-sheet-width");
    if (savedWidth) {
      try {
  ...
  useEffect(() => {
    if (!isMobile && width !== DEFAULT_WIDTH) {
      localStorage.setItem("sticky-notes-sheet-width", width.toString());
    }
    // On mobile, use full width; on desktop, use saved width
    const effectiveWidth = isMobile ? window.innerWidth : width;
    setStickyNotesWidth(open ? effectiveWidth : 0);
    setIsStickyNotesOpen(open);
  }, [width, open, isMobile, setStickyNotesWidth, setIsStickyNotesOpen]);
