...
export const getDeliveryWithItems = query({
    ...
    handler: async (ctx, args) => {
        ...
        // If it's a direct DC, we construct items directly from directItems
        if (delivery.isDirect && delivery.directItems) {
            let vendor = null;
            if (delivery.vendorId) {
                const v = await ctx.db.get(delivery.vendorId);
                if (v) { vendor = v; }
            }
            const creator = await ctx.db.get(delivery.createdBy);
            return {
                ...delivery,
                items: delivery.directItems.map((item, i) => ({
                    _id: `dc-item-${i}`,
                    ...
                    status: "delivered", // For display purposes
...
// ============================================================================
// Direct DC (Delivery Challan) — No request linkage
// ============================================================================

/**
 * Create a Direct DC — a standalone Delivery Challan not linked to any request or PO.
 * Stores item details directly in the deliveries table via `directItems`.
 * Restricted to purchase_officer role.
 */
export const createDirectDC = mutation({
    args: {
        ...
        deliveryContact: v.optional(v.string()),
        vehicleNumber: v.optional(v.string()),
        transportName: v.optional(v.string()),
        receiverName: v.string(),
        purchaserName: v.string(),
        dcDate: v.optional(v.number()),
    },
    handler: async (ctx, args) => {
        const user = await ctx.auth.getUserIdentity();
        ...
        if (!officer) throw new ConvexError("User not found");
        if (officer.role !== "purchase_officer" && officer.role !== "manager") {
            throw new ConvexError("Unauthorized: Only Purchase Officers and Managers can create Direct DCs");
        }
        ...
        for (const item of args.items) {
            if (item.quantity <= 0) throw new ConvexError(`Invalid quantity for "${item.itemName}"`);
            if (!item.itemName.trim()) throw new ConvexError("Item name is required");
        }
        ...
        const deliveryId = await ctx.db.insert("deliveries", {
            ...
            deliveryPerson: args.deliveryPerson,
            ...
            dcDate: args.dcDate || now,
            directItems: args.items,
            createdBy: officer._id,
            createdAt: now,
            updatedAt: now,
        });

        return { deliveryId, dcNumber: deliveryIdStr };
    },
});

/**
 * Get all Direct DC deliveries for display in the Requests list.
 * Returns Direct DCs with vendor and creator info enriched.
 */
export const getDirectDCDeliveries = query({
    args: {},
    handler: async (ctx) => {
        const user = await ctx.auth.getUserIdentity();
        if (!user) return [];
        ...
        // Only purchase officers and managers can view these
        if (currentUser.role !== "purchase_officer" && currentUser.role !== "manager") return [];
        ...
        // Enrich with vendor + creator info
