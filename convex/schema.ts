...
export default defineSchema({
  ...
  costComparisons: defineTable({
    requestId: v.optional(v.id("requests")), // Linked request
    // ── Direct CC Fields ──
    isDirect: v.optional(v.boolean()), // Flag for Direct CC (not linked to any request)
    ...
    })), // Standalone item data for Direct CC
    createdBy: v.id("users"), // Purchase Officer
  ...
  deliveries: defineTable({
    ...
    // ── Direct DC Fields ──
    isDirect: v.optional(v.boolean()), // Flag for Direct DC (not linked to any request)
    vendorId: v.optional(v.id("vendors")), // Direct vendor reference (for Direct DC)
    dcDate: v.optional(v.number()), // Custom DC date (defaults to createdAt)
    directItems: v.optional(v.array(v.object({
      itemName: v.string(),
      description: v.optional(v.string()),
      quantity: v.number(),
      unit: v.string(),
      hsnSacCode: v.optional(v.string()),
    }))), // Standalone item data for Direct DC (no request linkage)
